...
class TicketSubmissionForm(tk.Frame):
    ...
    def submit_ticket(self):
        ...
        self.master.submit_ticket(name, department, workstation, description)
...
class AdminPanel(tk.Frame):
    ...
    def update_status(self):
        ...
        if new_status:
            self.master.update_ticket_status(ticket_id, new_status)
            self.load_tickets()

    def delete_ticket(self):
        selected_ticket = self.ticket_list.get(tk.ACTIVE)
        ticket_id = int(selected_ticket.split(",")[0].split(":")[1].strip())
        self.master.delete_ticket(ticket_id)
        self.load_tickets()
...
class MainApplication(tk.Tk):
    ...
    def submit_ticket(self, name, department, workstation, description):
        conn = sqlite3.connect('support_db.sqlite')
        cursor = conn.cursor()
        cursor.execute('''
